<script lang='ts'>
	...
	import { available_files, file_server_port, is_loading_image_data, working_folder } from '$lib/stores';
	import { getAvailableFiles, getServedDir, selectFolder } from '$lib/bindings';
	...
	onMount(async () => {
		...
		// Handle vite hot reload by clearing the interval
...
<AppShell>
	...
	<svelte:fragment slot="sidebarLeft">
		<div class="h-full bg-surface-50-900-token border-r border-surface-500/30 w-[300px] overflow-hidden p-2 py-2">
			...
			<label class="label">
				...
				<input class="input variant-form-material" type="search" disabled placeholder="Filepath" title="Choose the base directory" bind:value={$working_folder} />
			</label>
			<div class="flex p-4">
				<button class="variant-filled-secondary btn variant-filled mx-auto w-full" on:click={select_new_folder}>Change</button>
			</div>
